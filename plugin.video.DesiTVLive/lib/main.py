...
class Main:
    ...
    def __init__(self):
        ...
        common.log('DesiTVLive initialized')
    ...
    def downloadVideo(self, url, title):
        ...
        title = getKeyboard(default = fu.cleanFilename(title),heading='DesiTVLive')
    ...
    def downloadCustomModule(self):
        ...
        if success == True:            
            # refresh container if DesiTVLive is active
            currContainer = xbmcUtils.getContainerFolderPath()
            common.showNotification('DesiTVLive', 'Download successful', 1000)
            if currContainer.startswith(self.base):                
                xbmc.executebuiltin('Container.Refresh()')
            return True
        elif success == False:        
            common.showNotification('DesiTVLive', 'Download failed', 1000)
        return False
        ...
        if definedIn:
            ...
            if definedIn.endswith('favourites.cfg') or definedIn.startswith("favfolders/"):
                ...
                if lItem['title'] != "Favourites":
                        # Add to favourites
                        contextMenuItem = createContextMenuItem('Add to DesiTVLive favourites', Mode.ADDTOFAVOURITES, codedItem)
                        contextMenuItems.append(contextMenuItem)
    ...
    def update(self):
        ...
        def checkForUpdates():
            updates = {}          
            common.showNotification('DesiTVLive', common.translate(30275))
        ...
        def doUpdates(typeName, updates):
            ...
            head = "DesiTVLive Updates - %s" % typeName
        ...
        if count == 0:
            common.showNotification('DesiTVLive', common.translate(30273))
            return
        else:
    ...
    def queueAllVideos(self, item):
        dia = DialogProgress()
        dia.create('DesiTVLive', 'Get videos...' + item['title'])
        dia.update(0)
    ...
    def run(self, argv=None):
        ...
        self.addon = Addon('plugin.video.DesiTVLive', argv)

        common.log('DesiTVLive running')
        ...
        try:
            ...
            common.showError('Error running DesiTVLive')
            common.log('Error running DesiTVLive. Reason:' + str(e))
